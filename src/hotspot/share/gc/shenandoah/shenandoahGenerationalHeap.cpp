...
// setting the old-generation balance so that we can expand or shrink the old generaetion.
//
...
// for the idle span that precedes the next GC.  The mutator_xfer_limit constraints the transfer of memory
// from young to old.  It does not constraint young reserves.
void ShenandoahGenerationalHeap::compute_old_generation_balance(size_t mutator_xfer_limit,
  ...
  // We allow young_reserve to exceed mutator_xfer_limit. Essentially, this means GC is already behind allocation
  // pace and we'll need to trigger the next GC as soon as possible.  But it is essential to provide reserves in order
  // for the urgent GC to be able to reclaim memory.
